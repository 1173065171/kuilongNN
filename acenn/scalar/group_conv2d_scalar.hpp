...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template<typename T>
acenn_matrix<T> *group_conv2d(uint8_t stride, uint8_t padding, acenn_matrix<T> *fm, acenn_matrix<T> *kernel, uint8_t shift_fig);
			...
			for (uint32_t row = 0; row < output->rows; ++row) {
				for (uint32_t col = 0; col < output->cols; ++col) {
					...
					for (uint32_t ic = 0; ic < kernel->channels; ++ic) {
						...
						T kernel_value = kernel->matrix[(ic)];
						sum += input_value * kernel_value;

						(output->matrix)[(ic * out_rows * out_cols) + (row * out_cols) + col] = static_cast<T>((sum >> shift_fig) & 0xFF);
					}
				}
			}
		}
